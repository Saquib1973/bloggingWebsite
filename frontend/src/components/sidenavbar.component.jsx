...
const SideNav = () => {
    ...
    const [page, setPage] = useState(path.replace('-', ' '));
    ...
    return (
        access_token === null ? <Navigate to={'/auth/signin'} /> :
            <>
                <section className='relative flex gap-10 py-0  m-0 max-md:flex-col'>
                    <div className='sticky top-[80px] z-30'>
                        <div className='md:hidden bg-white py-1 border-b border-grey flex flex-nowrap overflow-x-auto'>
                            ...
                            <hr ref={activeTabLine} className='absolute bottom-0 border-dark-grey duration-300' />
                        </div>
                        <div className={`min-w-[200px] h-[calc(100vh-80px-60px)] md:h-cover md:sticky top-24 overflow-y-auto p-6 md:pr-0 md:border-grey  md:border-r absolute max-md:top-[64px] bg-white  max-md:w-[calc(100%+80px)] max-md:px-16 max-md:-ml-7 duration-500 ${!showSideNav ? "max-md:opacity-0 max-md:pointer-events-none" : "opacity-100 pointer-events-auto"}`}>
                            ...
                            <h1 className='mt-20 text-xl text-dark-grey mb-3'>
                                Settings
                            </h1>
                            <hr className='border-grey -ml-6 mr-6 mb-8' />
                            <NavLink to={'/settings/edit-profile'} onClick={(e) => setPage(e.target.innerText)} className="sidebar-link">
                                <i className="fi fi-rr-user"></i>
                                Edit Profile</NavLink> <NavLink to={'/settings/change-password'} onClick={(e) => setPage(e.target.innerText)} className="sidebar-link">
